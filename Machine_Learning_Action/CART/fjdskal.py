...
def getMean(tree):
    ...
    Modify:
        2017-12-14
    """
    if is_dict(tree['right']): tree['right'] = getMean(tree['right'])
    if is_dict(tree['left']): tree['left'] = getMean(tree['left'])
...
def prune(tree, test_data):
    ...
    """
    # 如果测试集为空,则对树进行塌陷处理
    if np.shape(test_data)[0] == 0: return getMean(tree)
    # 如果有左子树或者右子树,则切分数据集
    if is_dict(tree['right']) or is_dict(tree['left']):
        l_set, r_set = binSplitDataSet(test_data, tree['spInd'], tree['spVal'])
    # 处理左子树(剪枝)
